...
const JG_AJAX_POST_FORM_METHOD = 'jg_ajax_get' //method used for forms that should be via ajax post
    ...
    forms.map((form)=>{
      //add onsubmit event listener to form to submit form via ajax
        form.addEventListener('submit', async (event)=>{
            ...
            const handler = form.getAttribute(JG_AJAX_RESPONSE_HANDLER_ATTRNAME)
            if (handler && typeof window[handler] === 'function') eval(handler)(event, data)
        });
    });
}
/*
